...
def VT_check(url):
    ansstring = '\nАнализ VirusTotal (только сомнительные результаты):\n'
    ...
    if response.status_code == 200:
        ...
        for key in result['data']['attributes']['last_analysis_results']:
            if result['data']['attributes']['last_analysis_results'][key]['result'] != 'clean':
                ansstring += key +'\n'
                ansstring += '  Category: ' + result['data']['attributes']['last_analysis_results'][key]['category']+ '\n'
                ansstring += '  Result: ' + result['data']['attributes']['last_analysis_results'][key]['result'] +'\n'
                ansstring += '  Method: ' + result['data']['attributes']['last_analysis_results'][key]['method'] + '\n'
    return ansstring
